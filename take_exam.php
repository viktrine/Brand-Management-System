...
<?php $query1 = mysql_query("select * from student_class_exam where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ")or die(mysql_error());
...
if ($count > 0){
 ...
 mysql_query("insert into student_class_exam (class_quiz_id,student_id,student_quiz_time) values('$class_quiz_id','$session_id','$quiz_time')");
}
 ?>
...
$sqlp = mysql_query("SELECT * FROM class_exam WHERE class_quiz_id = '$class_quiz_id'")or die(mysql_error());
...
<form action="take_exam.php<?php echo '?id='.$get_id; ?>&<?php echo 'class_quiz_id='.$class_quiz_id; ?>&<?php echo 'exam=done' ?>&<?php echo 'quiz_id='.$quiz_id; ?>&<?php echo 'quiz_time='.$quiz_time; ?>" name="testform" method="POST" id="test-form">
<?php
										$sqla = mysql_query("select * FROM class_exam 
										LEFT JOIN exam ON exam.quiz_id  = class_exam.quiz_id
...
?>
										<h3>Exam Title: <b><?php echo $rowa['quiz_title'];?></b></h3>
...
<?php
	$sqlw = mysql_query("SELECT * FROM exam_question where quiz_id = '$quiz_id'  ORDER BY RAND()");
...
<?php
	...
	for($x=1;$x<=$x1;$x++){
		...
		$sql = mysql_query("SELECT * FROM exam_question WHERE quiz_question_id = ".$x2."");
	...
	mysql_query("UPDATE student_class_exam SET `student_quiz_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'")or die(mysql_error());
?>
<script>
